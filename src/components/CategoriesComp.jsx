...
const CategoriesComp = () => {
  const { categories, navigate } = useCategories();
      ...
      <div className="flex justify-center items-center text-white w-full relative  lg:h-32  bottom-0 z-10 lg:pr-32">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 -mt-64 lg:-mt-96 gap-6 h-full w-full md:px-20 lg:pl-32 lg:px-0">
          {categories.slice(2, 6).map((item, index) => (
            <div
              onClick={() => {
                if (index === categories.slice(2, 6).length - 1) {
                  // Jika index adalah indeks terakhir, arahkan ke "/categories"
                  navigate("/categories");
                } else {
                  navigate(`/destinationbycategories/${item?.id}`);
                }
              }}
              className={`h-40 w-2/3 md:w-full mx-auto rounded-md hover:scale-105 transition flex items-end cursor-pointer
              }`}
              style={{
